...
def train_model(initial_params: Optional[TrainingParameters] = None):
    ...
    for epoch in range(params.epochs):
        ...
        # Log to CSV
        job_logger.log_epoch(
            job_id=f"zpe_job_{int(time.time())}",
            epoch=epoch + 1,
            ...
            val_accuracy=val_acc,
            epoch_time=epoch_time,
            conv_channels=[50, 52, 123, 255],  # Example values, adjust as needed
            fc_hidden=811,
            dropout_fc=0.254806779712836,
            energy_scale=1.00797735672017,
            learning_rate=params.learning_rate,
            mixup_alpha=0.471825751575784,
            batch_size=params.batch_size,
            momentum_factor=0.233421746798178,
            label_smoothing=params.label_smoothing,
            quantum_circuit_size=16,
            quantum_mode=True,
            momentum_params=params.momentum_params,
            strength_params=params.strength_params,
    ...
    job_logger.log_job_completion(job_data)
