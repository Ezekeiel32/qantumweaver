              ...
              <CardContent className="pt-0">
                  <div className="h-[350px]">
                    <NeonAnalyzerChart
                      traces={[
                        {
                          data: getTrainingCurveData().map(d => metricType === 'accuracy' ? d[Object.keys(d).find(k => k.includes('Val Acc'))!] : d[Object.keys(d).find(k => k.includes('Val Loss'))!]),
                          color: metricType === 'accuracy' ? '#ffe066' : '#00ffe7',
                          label: metricType === 'accuracy' ? 'Validation Accuracy' : 'Validation Loss',
                        },
                        ...(comparisonJob ? [{
                          data: getTrainingCurveData().map(d => metricType === 'accuracy' ? d[Object.keys(d).find(k => k.includes('Val Acc') && !k.includes(activeJob?.parameters.modelName || 'Active'))!] : d[Object.keys(d).find(k => k.includes('Val Loss') && !k.includes(activeJob?.parameters.modelName || 'Active'))!]),
                          color: '#ff00ff',
                          label: 'Comparison',
                        }] : [])
                      ]}
