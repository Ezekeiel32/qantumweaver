              ...
              {isMonitorLoading ? (
                ...
                <div className="space-y-6">
                  ...
                  <div className="h-[300px]">
                    {(() => {
                      const accuracyData = chartData.map(d => typeof d.accuracy === 'number' ? d.accuracy : null).filter(v => v !== null);
                      const lossData = chartData.map(d => typeof d.loss === 'number' ? d.loss : null).filter(v => v !== null);
                      const avgZpeData = chartData.map(d => typeof d.avg_zpe === 'number' ? d.avg_zpe : null).filter(v => v !== null);
                      const missingTraces = [];
                      if (accuracyData.length === 0) missingTraces.push('Accuracy');
                      if (avgZpeData.length === 0) missingTraces.push('Avg ZPE');
                      return (
                        <>
                          <NeonAnalyzerChart
                            traces={[
                              { data: accuracyData, color: '#00ffe7', label: 'Accuracy' },
                              { data: lossData, color: '#ff00ff', label: 'Loss' },
                              { data: avgZpeData, color: '#ffe066', label: 'Avg ZPE' },
                            ]}
                            width={600}
                            height={300}
                            overlays={[
                              { text: 'Training Progress', color: '#39ff14', x: 120, y: 40, fontSize: 24 },
                              { text: activeJob?.parameters?.modelName || '', color: '#00ffe7', x: 80, y: 280, fontSize: 16 },
                              ...(missingTraces.length > 0 ? [{ text: `Missing: ${missingTraces.join(', ')}`, color: '#ff3c3c', x: 200, y: 60, fontSize: 18 }] : [])
                            ]}
                            showLegend
                          />
                        </>
