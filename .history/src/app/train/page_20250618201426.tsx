...
export default function TrainModelPage() {
  ...
  const { control, handleSubmit, reset, watch, formState: { errors }, setValue, getValues } = useForm<TrainingParameters>({
  ...
  return (
    <>
      {/* Main content area: vertical flex, chart on top, logs at bottom, sidebar untouched */}
      <div className="min-h-screen w-full flex flex-row bg-gradient-to-br from-[#181c24] to-[#23283a] bg-noise-pattern">
        {/* Sidebar is rendered elsewhere, do not touch */}
        {/* Main vertical stack to the right of sidebar */}
        <div className="flex-1 flex flex-col items-center justify-start px-2 py-4">
          <div className="w-full max-w-6xl flex flex-col h-[90vh]" style={{ minHeight: '90vh' }}>
            {/* Training Monitor (chart + metrics bar) at the top */}
            <div className="w-full crt-panel neon-border flex flex-col justify-start p-6 md:p-8" style={{ height: '794px', minHeight: '340px', maxHeight: 'calc(75vh + 144px)', borderRadius: '18px', boxShadow: '0 0 32px #00ffe7cc, 0 0 8px #00ffe799' }}>
              <div className="crt-scanlines" />
              <div className="crt-grid" />
              {/* Header row: title left, config button right */}
              <div className="flex items-center justify-between mb-2">
                <div className="flex items-center gap-3 text-2xl font-headline text-primary drop-shadow-neon">
                  <List className="h-7 w-7 text-accent" />
                  <span>Training Monitor</span>
                </div>
                <Dialog open={configDialogOpen} onOpenChange={setConfigDialogOpen}>
                  <DialogTrigger asChild>
                    <button className="px-6 py-2 rounded-xl neon-border bg-zinc-900/80 text-neon-cyan font-bold text-lg shadow-lg hover:bg-zinc-800/90 transition" style={{ marginRight: 0 }}>Training Configuration</button>
                  </DialogTrigger>
                  <DialogContent className="dialog-content minimal-scrollbar" style={{ minWidth: '900px', maxWidth: '1200px', width: '90vw', minHeight: '600px', maxHeight: '90vh', overflowY: 'auto' }}>
                    <div className="scanline-bg" />
                    <DialogTitle className="modal-title">Training Configuration</DialogTitle>
                    {/* Render the full configuration form inside the modal */}
              ...
              {/* Status/metrics bar: grid layout for alignment */}
              <div className="grid grid-cols-5 gap-4 px-4 py-2 mb-2 bg-black/30 rounded-lg border border-neon-cyan text-neon-cyan text-base font-mono">
                <span className="crt-flicker">{activeJob?.status?.toUpperCase() || 'NONE'}</span>
                <span>Epoch: {activeJob?.current_epoch ?? '--'}/{activeJob?.total_epochs ?? '--'}</span>
                <span>Accuracy: {activeJob ? `${activeJob.accuracy.toFixed(2)}%` : '--'}</span>
                <span>Loss: {activeJob ? activeJob.loss.toFixed(4) : '--'}</span>
                <span>Avg ZPE: {activeJob?.zpe_effects && activeJob.zpe_effects.length > 0 ? (activeJob.zpe_effects.reduce((a, b) => a + b, 0) / activeJob.zpe_effects.length).toFixed(2) : '--'}</span>
              </div>
              {/* Chart area below */}
              <div className="flex-1 w-full min-h-0 flex flex-col items-start minimal-scrollbar">
            ...
            {/* Gap between chart and terminal */}
            <div className="mt-6" />
            {/* Training Logs/Terminal directly underneath, fills remaining space, no CRT overlays */}
            <div className="w-[96%] mx-auto flex flex-col overflow-y-auto neon-border" style={{ background: '#0a0f1c', minHeight: 'calc((1.5em * 17) - 144px)', maxHeight: 'calc((1.5em * 17) - 144px)', color: '#39ff14', fontFamily: 'Share Tech Mono, VT323, Space Mono, monospace', borderRadius: '18px', boxShadow: '0 0 32px #00ffe7cc, 0 0 8px #00ffe799' }}>
              {/* Logs header bar */}
              <div className="flex items-center gap-2 px-4 py-2 border-b border-neon-cyan bg-black/40" style={{ borderTopLeftRadius: '18px', borderTopRightRadius: '18px' }}>
                <BarChart3 className="h-5 w-5 text-accent" />
                <span className="font-bold text-base text-neon-cyan drop-shadow-neon">Training Logs</span>
              </div>
              <div className="flex-1 overflow-y-auto space-y-1 minimal-scrollbar p-2" style={{ maxHeight: 'calc(1.5em * 17)', minHeight: 'calc(1.5em * 17)' }}>
                {activeJob?.log_messages?.slice(-17).map((msg, idx, arr) => {
                  let color = '#39ff14';
                  let shadow = '0 0 6px #39ff14';
