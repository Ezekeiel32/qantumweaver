...
import { Play, StopCircle, List, Zap, Settings, RefreshCw, AlertTriangle, ExternalLink, SlidersHorizontal, Atom, Brain, Waves, BrainCircuit, Wand2, Save, Download, ArrowDownCircle, PlayCircle, ChevronLeft, ChevronRight, Maximize2, Minimize2, BarChart3, X } from "lucide-react";
...
function TrainModelPage() {
  ...
  return (
    <div className="relative flex flex-col h-full min-h-screen w-full">
      ...
      <div className="matrix-panel mb-2" style={{ minHeight: '740px', height: '78vh', display: 'flex', flexDirection: 'column', justifyContent: 'flex-start' }}>
        <div className="flex items-center justify-between mb-2">
          <h2 className="text-2xl font-headline neon-heading flex items-center gap-2">
            <List className="h-7 w-7 text-accent" /> Training Monitor
          </h2>
          {/* Config open button on far right */}
          <Button
            variant="ghost"
            className="ml-auto neon-btn px-4 py-2 text-base font-semibold"
            style={{ position: 'absolute', right: 32, top: 24, zIndex: 10 }}
            onClick={() => setShowConfigModal(true)}
            aria-label="Open Training Configuration"
          >
            <Settings className="h-6 w-6 mr-2" /> Configure
          </Button>
        </div>
        {/* Training Progress Bar and Chart */}
        <div className="w-full flex flex-col flex-1">
          {/* Progress and stats row */}
          <div className="flex items-center gap-3 mb-4">
            <span className="font-mono text-lg text-accent-foreground">Training Progress</span>
          ...
          {/* Chart centered, with blue neon border visible on all sides, black bg inset by 8px (1 grid pixel) */}
          <div className="flex-1 w-full flex items-center justify-center" style={{ minHeight: 380 }}>
            <div
      ...
      {/* Training Logs directly under monitor, not at the very bottom */}
      <div className="w-full mb-16">
        <div className="matrix-panel" style={{ minHeight: 220, maxHeight: 340, overflowY: 'auto' }}>
          <h3 className="text-lg font-headline mb-2 flex items-center gap-2"><BarChart3 className="h-5 w-5 text-accent" /> Training Logs</h3>
      ...
      {showConfigModal && (
        <div className="modal-overlay" onClick={() => setShowConfigModal(false)}>
          <div className="modal-content" onClick={e => e.stopPropagation()}>
            <Button
              variant="ghost"
              className="absolute top-4 right-4 neon-btn"
              onClick={() => setShowConfigModal(false)}
              aria-label="Close Configuration"
            >
              <X className="h-6 w-6" />
            </Button>
            <h2 className="text-2xl font-headline neon-heading mb-4 flex items-center gap-2"><Settings className="h-7 w-7 text-accent" /> Training Configuration</h2>
                ...
                <TabsContent value="general">
                  <div className="grid gap-4">
                    ...
                    <div className="flex flex-col gap-2 mt-2">
                      <Label className="text-sm font-mono text-neon-cyan">Mixup Alpha (Noise Strength)</Label>
                      <div className="flex items-center gap-4">
                        ...
                        <Button type="button" variant="secondary" onClick={handleSweepMixupAlpha} className="ml-2" disabled={sweeping}>{sweeping ? 'Sweeping...' : 'Sweep'}</Button>
                      </div>
...
export default function TrainModelPageWithSuspense() {
  return (
    <Suspense>
      <TrainModelPage />
    </Suspense>
