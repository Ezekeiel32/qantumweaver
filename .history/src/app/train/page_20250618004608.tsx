...
import { Play, StopCircle, List, Zap, Settings, RefreshCw, AlertTriangle, ExternalLink, SlidersHorizontal, Atom, Brain, Waves, BrainCircuit, Wand2, Save, Download, ArrowDownCircle, PlayCircle, ChevronLeft, ChevronRight, Maximize2, Minimize2 } from "lucide-react";
...
export default function TrainModelPage() {
  ...
  // --- New state for resizable monitor and minimizable config ---
  const [monitorWidth, setMonitorWidth] = useState(700); // px, default
  const [trainConfigMinimized, setTrainConfigMinimized] = useState(false);
  const monitorMinWidth = 480;
  const monitorMaxWidth = 1200;
  const dragHandleWidth = 10;
  const monitorRef = useRef<HTMLDivElement>(null);
  const [draggingMonitor, setDraggingMonitor] = useState(false);
  ...
  useEffect(() => {
    if (!monitorMaximized) return;
    function updateChartDims() {
      if (maximizedChartContainerRef.current) {
        const width = maximizedChartContainerRef.current.offsetWidth;
        const height = Math.max(340, window.innerHeight * 0.45);
    ...
    if (typeof window !== 'undefined') {
      ...
      if (maximizedChartContainerRef.current && typeof ResizeObserver !== 'undefined') {
        ro = new ResizeObserver(updateChartDims);
        ro.observe(maximizedChartContainerRef.current);
      }
      return () => {
        window.removeEventListener('resize', updateChartDims);
        if (ro && maximizedChartContainerRef.current) ro.disconnect();
      };
    }
  }, [monitorMaximized]);

  // --- Drag logic for monitor width ---
  React.useEffect(() => {
    if (!draggingMonitor) return;
    const onMouseMove = (e: MouseEvent) => {
      if (dragStartX.current !== null && dragStartWidth.current !== null) {
        const dx = e.clientX - dragStartX.current;
        let newWidth = dragStartWidth.current + dx;
        newWidth = Math.max(monitorMinWidth, Math.min(monitorMaxWidth, newWidth));
        setMonitorWidth(newWidth);
      }
    };
    const onMouseUp = () => {
      setDraggingMonitor(false);
    ...
    <>
      ...
      <div className="container mx-auto p-4 md:p-6">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          {!trainConfigMinimized && !monitorMaximized && (
            <Card className="w-full relative" style={{ minWidth: 320, maxWidth: 480, zIndex: 20 }}>
              <CardHeader>
                ...
                <button
                  onClick={() => setTrainConfigMinimized(true)}
                  style={{
                    position: "absolute",
                    left: -22,
                    bottom: 18,
          ...
          {/* Training Monitor Panel (Resizable/Maximizable) */}
          {!monitorMaximized && (
            <div
              ref={monitorRef}
              className="relative"
              style={{ width: monitorWidth, minWidth: monitorMinWidth, maxWidth: monitorMaxWidth, transition: draggingMonitor ? "none" : "width 0.2s", zIndex: 10 }}
            >
              <Card className="w-full">
                ...
                <CardContent style={{ paddingTop: 8, paddingBottom: 8 }}>
                  {/* Compact status/metrics bar at the very top */}
                  <div style={{
                    display: 'flex',
                    alignItems: 'center',
                    ...
                    <span style={{ color: '#00ffe7', fontFamily: 'Share Tech Mono, monospace', fontSize: 18, letterSpacing: 1.2 }}>Status</span>
                    <span style={{ background: 'rgba(0,255,255,0.08)', border: '1.5px solid #00ffe7', borderRadius: 16, color: '#00ffe7', fontFamily: 'Share Tech Mono, monospace', fontSize: 17, padding: '2px 24px', boxShadow: '0 0 12px #00ffe7', marginRight: 8 }}>{activeJob?.status?.toUpperCase() || 'NONE'}</span>
                    <div style={{ display: 'flex', gap: 18, flex: 1, justifyContent: 'center', minWidth: 0 }}>
                      <div style={{ background: 'rgba(255,255,255,0.03)', borderRadius: 10, padding: '4px 24px', minWidth: 110, textAlign: 'center', border: '1px solid #222' }}>
                        <div style={{ fontSize: 13, color: '#b0b0b0', marginBottom: 0 }}>Epoch</div>
                        <div style={{ fontWeight: 700, fontSize: 22, letterSpacing: 1 }}>{activeJob?.current_epoch ?? '--'}/{activeJob?.total_epochs ?? '--'}</div>
                      </div>
                      <div style={{ background: 'rgba(255,255,255,0.03)', borderRadius: 10, padding: '4px 24px', minWidth: 110, textAlign: 'center', border: '1px solid #222' }}>
                        <div style={{ fontSize: 13, color: '#b0b0b0', marginBottom: 0 }}>Accuracy</div>
                        <div style={{ fontWeight: 700, fontSize: 22, letterSpacing: 1 }}>{activeJob ? `${activeJob.accuracy.toFixed(2)}%` : '--'}</div>
                      </div>
                      <div style={{ background: 'rgba(255,255,255,0.03)', borderRadius: 10, padding: '4px 24px', minWidth: 110, textAlign: 'center', border: '1px solid #222' }}>
                        <div style={{ fontSize: 13, color: '#b0b0b0', marginBottom: 0 }}>Loss</div>
                        <div style={{ fontWeight: 700, fontSize: 22, letterSpacing: 1 }}>{activeJob ? activeJob.loss.toFixed(4) : '--'}</div>
                      </div>
                      <div style={{ background: 'rgba(255,255,255,0.03)', borderRadius: 10, padding: '4px 24px', minWidth: 110, textAlign: 'center', border: '1px solid #222' }}>
                        <div style={{ fontSize: 13, color: '#b0b0b0', marginBottom: 0 }}>Avg ZPE</div>
                        <div style={{ fontWeight: 700, fontSize: 22, letterSpacing: 1 }}>{activeJob?.zpe_effects && activeJob.zpe_effects.length > 0 ? (activeJob.zpe_effects.reduce((a, b) => a + b, 0) / activeJob.zpe_effects.length).toFixed(2) : '--'}</div>
                      </div>
                    </div>
                    {activeJob?.status === 'running' && (
                      <button
                        onClick={() => handleStopJob(activeJob.job_id)}
                        style={{
                        ...
                        title="Stop Job"
                      >
                        <StopCircle className="mr-1 h-5 w-5" /> Stop Job
                      </button>
                    )}
                  </div>
                  {/* Chart and logs fill all available space */}
                  <div style={{ flex: 1, display: 'flex', flexDirection: 'column', minHeight: 0, minWidth: 0, padding: '0 32px 24px 32px' }}>
                    <div ref={maximizedChartContainerRef} style={{ flex: 2, minHeight: 0, minWidth: 0, marginTop: 8, marginBottom: 8, width: '100%' }}>
                      <NeonAnalyzerChart
                        traces={[
                    ...
                    <ScrollArea className="rounded-md border bg-[#18181b] retro-terminal overflow-auto relative" style={{ flex: 1, minHeight: 0, minWidth: 0, fontFamily: '"Fira Mono", "JetBrains Mono", "IBM Plex Mono", monospace', boxShadow: '0 0 24px #00ffcc44', border: '2px solid #00ffcc', background: 'repeating-linear-gradient(0deg, #18181b 0px, #18181b 2px, #1a1a1e 4px, #18181b 6px)', color: '#00ffcc', marginTop: 8 }}>
                      <div className="p-4" ref={retroLogRef} id="retro-log-container" style={{ height: '100%', overflowY: 'auto', position: 'relative' }}>
                        <h4 className="mb-4 text-sm font-medium leading-none text-[#00ffcc] drop-shadow-[0_0_4px_#00ffcc]">Training Logs</h4>
                        <div style={{ fontSize: '1.05em', lineHeight: '1.45', letterSpacing: '0.01em' }}>
                          {activeJob?.log_messages?.map((msg, idx, arr) => {
                            let color = '#00ffcc';
              ...
              {/* Drag handle for resizing (right edge) */}
              <div
                style={{
                  position: "absolute",
                  top: 0,
                  ...
                  cursor: "ew-resize",
                  zIndex: 30,
                  background: draggingMonitor ? "#00ffe7AA" : "#00ffe733",
                  borderRadius: 6,
                  boxShadow: draggingMonitor ? "0 0 12px #00ffe7" : "none",
                  transition: "background 0.2s",
                }}
                onMouseDown={e => {
                  setDraggingMonitor(true);
                  dragStartX.current = e.clientX;
                  dragStartWidth.current = monitorWidth;
                }}
                title="Resize monitor width"
              />
              {/* If config is minimized, show the floating chevron button attached to bottom left */}
              {trainConfigMinimized && (
                <div style={{ position: "absolute", left: -54, bottom: 18, zIndex: 50 }}>
                  <button
                    onClick={() => setTrainConfigMinimized(false)}
                    style={{
                      background: "#10131a",
                      border: "2px solid #00ffe7",
                      borderRadius: "50%",
                      boxShadow: "0 0 16px #00ffe7, 0 2px 8px #0008",
                      width: 44,
                      height: 44,
                      display: "flex",
                      alignItems: "center",
                    ...
                    title="Restore configuration panel"
                  >
                    <ChevronRight className="h-7 w-7" style={{ color: "#00ffe7", filter: "drop-shadow(0 0 8px #00ffe7)" }} />
                  </button>
                </div>
              )}
            </div>
          )}
          {/* Maximized Monitor Overlay */}
          {monitorMaximized && (
            <div
              style={{
                position: 'fixed',
                ...
                boxShadow: '0 0 64px #00ffe7, 0 2px 64px #0008',
                borderRadius: 18,
                display: 'flex',
            ...
            >
              <div style={{ display: 'flex', alignItems: 'center', padding: '12px 24px 0 24px', minHeight: 56 }}>
                <span className="flex items-center gap-2 text-lg" style={{ color: '#00ffe7', fontFamily: 'Share Tech Mono, monospace', fontSize: 22, letterSpacing: 1.2 }}>
                  <List className="h-6 w-6 text-primary" /> Training Monitor
                </span>
                <button
                  onClick={() => setMonitorMaximized(false)}
                  style={{
                    marginLeft: 'auto',
                    background: '#10131a',
                    border: '2px solid #00ffe7',
                ...
                >
                  <Minimize2 className="h-6 w-6" style={{ color: '#00ffe7' }} />
                </button>
              </div>
              {/* Compact status/metrics bar at the very top of overlay */}
              <div style={{
                display: 'flex',
                alignItems: 'center',
                justifyContent: 'space-between',
              ...
              {/* Chart and logs fill all available space */}
              <div style={{ flex: 1, display: 'flex', flexDirection: 'column', minHeight: 0, minWidth: 0, padding: 0, width: '100%' }}>
                <div ref={maximizedChartContainerRef} style={{ flex: 2, minHeight: 0, minWidth: 0, margin: 0, width: '100%', height: 0, flexGrow: 1, display: 'flex' }}>
                  <NeonAnalyzerChart
                    traces={[
                      { data: chartData.map(d => typeof d.accuracy === 'number' ? d.accuracy : null).filter(v => v !== null), color: '#00ffe7', label: 'Accuracy' },
                      { data: chartData.map(d => typeof d.loss === 'number' ? d.loss : null).filter(v => v !== null), color: '#ff00ff', label: 'Loss' },
                ...
                <ScrollArea className="rounded-md border bg-[#18181b] retro-terminal overflow-auto relative" style={{ flex: 1, minHeight: 0, minWidth: 0, fontFamily: '"Fira Mono", "JetBrains Mono", "IBM Plex Mono", monospace', boxShadow: '0 0 24px #00ffcc44', border: '2px solid #00ffcc', background: 'repeating-linear-gradient(0deg, #18181b 0px, #18181b 2px, #1a1a1e 4px, #18181b 6px)', color: '#00ffcc', margin: 0 }}>
                  <div className="p-4" ref={retroLogRef} id="retro-log-container" style={{ height: '100%', overflowY: 'auto', position: 'relative' }}>
                    <h4 className="mb-4 text-sm font-medium leading-none text-[#00ffcc] drop-shadow-[0_0_4px_#00ffcc]">Training Logs</h4>
                    <div style={{ fontSize: '1.05em', lineHeight: '1.45', letterSpacing: '0.01em' }}>
                      {activeJob?.log_messages?.map((msg, idx, arr) => {
