    ...
    <Card className="chart-panel h-full w-full bg-[#0a0f1c] border-neon-blue/30 shadow-neon-blue/20 flex flex-col" style={{ fontFamily: 'Share Tech Mono, Space Mono, VT323, monospace' }}>
      <div className="chart-header p-4 border-b border-neon-blue/20 relative">
        <div className="flex items-center justify-between">
          ...
          {/* HS-QNN Start Training Button */}
          {advisorParams && onStartAdvisorTraining && (
            <button
              className="instrument-btn ml-6 px-6 py-2 text-lg font-bold border-2 border-[#ffe066] text-[#ffe066] bg-[#18181b] shadow-[0_0_16px_#ffe066] hover:bg-[#ffe066] hover:text-[#18181b] transition-all rounded-full"
              style={{ position: 'absolute', top: 16, right: 16, zIndex: 30 }}
              onClick={() => onStartAdvisorTraining(advisorParams)}
            >
      ...
      <div ref={containerRef} className="flex-1 w-full h-full relative flex items-center justify-center p-0 m-0">
        <div className="absolute inset-0 grid-overlay opacity-20"></div>
        {!isLoading && !hasData && (
          <div className="absolute inset-0 flex items-center justify-center">
            <div className="text-neon-blue/70 font-mono text-lg">No training data yet</div>
          </div>
        )}
        {isLoading && !hasData && (
          <div className="absolute inset-0 flex items-center justify-center">
            <div className="text-neon-blue/70 font-mono text-lg animate-pulse">Initializing training...</div>
          </div>
        )}
        <div className="relative z-10 w-full h-full flex items-center justify-center">
          <NeonAnalyzerChart
            traces={[accuracyTrace, lossTrace, valAccuracyTrace, valLossTrace]}
            width={size.width}
            height={size.height}
            showLegend={true}
            xLabel="Epoch"
      ...
      <div className="absolute bottom-4 left-4 right-4">
        <div className="flex items-center justify-between bg-black/50 p-2 rounded border border-neon-blue/20">
          ...
          <div className="flex items-center space-x-2">
            ...
            <Button size="sm" variant="outline" className="h-6 px-2 border-neon-blue/30 text-neon-blue text-xs" onClick={resume}>
              RESUME
            </Button>
