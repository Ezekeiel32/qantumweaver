...
export default function NeonAnalyzerChart({
  ...
  function renderMiniMap() {
    ...
    return (
      <div style={{
        position: "absolute", bottom: 10, right: 10, width: 140, height: 36, background: "#10131aCC", border: "1.5px solid #00ffe7", borderRadius: 8, zIndex: 3, boxShadow: "0 0 4px #00ffe7", display: "flex", alignItems: "center", justifyContent: "center", padding: 2
      }}>
        <svg width={120} height={24}>
          {traces.map((trace, tIdx) => {
            ...
            const points = d.map((v, i) => {
              ...
              const y = 24 - ((v - minY) / (maxY - minY)) * 22;
              return `${x},${y}`;
            }).join(" ");
            return <polyline key={tIdx} points={points} fill="none" stroke={trace.color || defaultColors[tIdx % defaultColors.length]} strokeWidth={0.8} opacity={0.7} />;
          })}
          {/* Window rectangle */}
          <rect x={(startIdx / n) * 120} y={2} width={(windowSize / n) * 120} height={20} fill="#00ffe7" opacity={0.10} stroke="#00ffe7" strokeWidth={0.7} />
        </svg>
      </div>
    );
  }
  ...
  function renderMarkerTable() {
    ...
    return (
      <div style={{ position: "absolute", top: 10, right: 10, zIndex: 4, background: "#10131aCC", border: "1.5px solid #ffe066", borderRadius: 8, padding: "4px 10px", minWidth: 90, color: "#ffe066", fontFamily: "'Share Tech Mono', 'VT323', monospace", fontSize: 12, boxShadow: "0 0 4px #ffe066", textAlign: "right" }}>
        <b style={{ fontSize: 12 }}>Markers</b>
  ...
  function renderDeltaTable() {
    ...
    return (
      <div style={{ position: "absolute", top: 54, right: 10, zIndex: 4, background: "#10131aCC", border: "1.5px solid #39ff14", borderRadius: 8, padding: "4px 10px", minWidth: 90, color: "#39ff14", fontFamily: "'Share Tech Mono', 'VT323', monospace", fontSize: 12, boxShadow: "0 0 4px #39ff14", textAlign: "right" }}>
        <b style={{ fontSize: 12 }}>Delta</b>
  ...
  useEffect(() => {
    ...
    traces.forEach((trace, tIdx) => {
      ...
      // Thinner, zoom-responsive lines
      const baseLineWidth = 1.5;
      const lineWidth = Math.max(0.5, baseLineWidth / Math.sqrt(zoom));
      ...
      glowCtx.save();
      glowCtx.globalAlpha = 0.7;
      const markerRadius = Math.max(2, 5 / Math.sqrt(zoom));
      for (let i = startIdx; i < endIdx; i++) {
        const x = 50 + ((i - startIdx) / (windowSize - 1)) * (width - 70);
        const y = height - 30 - ((d[i] - minY) / (maxY - minY)) * (height - 40);
