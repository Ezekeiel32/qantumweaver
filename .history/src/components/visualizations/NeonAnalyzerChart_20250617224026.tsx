...
export default function NeonAnalyzerChart({
  ...
  useEffect(() => {
    const handleKey = (e: KeyboardEvent) => {
      if (e.key === "ArrowLeft") setPan(p => Math.max(0, p - 0.05));
      if (e.key === "ArrowRight") setPan(p => Math.min(1, p + 0.05));
  ...
  function renderMiniMap() {
    ...
    return (
      <div style={{
        position: "absolute", bottom: 10, left: 10, width: 160, height: 40, background: "#10131aAA", border: "1.5px solid #00ffe7", borderRadius: 8, zIndex: 3, boxShadow: "0 0 8px #00ffe7", display: "flex", alignItems: "center", justifyContent: "center"
      }}>
        <svg width={150} height={30}>
          {traces.map((trace, tIdx) => {
            ...
            const points = d.map((v, i) => {
              const x = (i / (d.length - 1)) * 150;
              const y = 30 - ((v - minY) / (maxY - minY)) * 28;
              return `${x},${y}`;
            }).join(" ");
            return <polyline key={tIdx} points={points} fill="none" stroke={trace.color || defaultColors[tIdx % defaultColors.length]} strokeWidth={1.5} opacity={0.7} />;
          })}
          {/* Window rectangle */}
          <rect x={(startIdx / n) * 150} y={2} width={(windowSize / n) * 150} height={26} fill="#00ffe7" opacity={0.13} stroke="#00ffe7" strokeWidth={1} />
        </svg>
      </div>
    );
  }
  ...
  function renderMarkerTable() {
    if (!showMarkerTable || measureMarkers.length === 0) return null;
    return (
      <div style={{ position: "absolute", top: 10, left: 10, zIndex: 4, background: "#10131aEE", border: "1.5px solid #ffe066", borderRadius: 8, padding: 8, minWidth: 120, color: "#ffe066", fontFamily: "'Share Tech Mono', 'VT323', monospace", fontSize: 14, boxShadow: "0 0 8px #ffe066" }}>
        <b>Markers</b>
  ...
  function renderDeltaTable() {
    if (!showDeltaTable || measureMarkers.length !== 2) return null;
    const [m1, m2] = measureMarkers;
    return (
      <div style={{ position: "absolute", top: 10, left: 150, zIndex: 4, background: "#10131aEE", border: "1.5px solid #39ff14", borderRadius: 8, padding: 8, minWidth: 120, color: "#39ff14", fontFamily: "'Share Tech Mono', 'VT323', monospace", fontSize: 14, boxShadow: "0 0 8px #39ff14" }}>
        <b>Delta</b>
        <div>ΔE: {Math.abs(m2.x - m1.x)}</div>
        <div>ΔY: {(m2.y - m1.y).toFixed(4)}</div>
      </div>
    );
  }
  ...
  useEffect(() => {
    ...
    traces.forEach((trace, tIdx) => {
      ...
      glowCtx.stroke();
      glowCtx.restore();
      // --- Per-epoch markers (clearer, with dots) ---
      glowCtx.save();
      glowCtx.globalAlpha = 0.7;
      for (let i = startIdx; i < endIdx; i++) {
        const x = 50 + ((i - startIdx) / (windowSize - 1)) * (width - 70);
  ...
  }, [traces, width, height, overlays, markers, gridColor, bgColor, showLegend, yMin, yMax, zoom, pan, measureMarkers, hoveredEpoch]);

  // --- Auto-scroll to latest epoch by default ---
  useEffect(() => {
    setPan(1); // Always show latest by default
