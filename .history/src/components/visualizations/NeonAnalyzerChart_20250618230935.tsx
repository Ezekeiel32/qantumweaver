...
export default function NeonAnalyzerChart({
  ...
  useEffect(() => {
    ...
    // Render math trace if set, on top
    const mathTrace = getMathTrace();
    const tracesToRender = [...traces];
    if (mathTrace) tracesToRender.push(mathTrace);
    tracesToRender.forEach((trace, tIdx) => {
      glowCtx.save();
      const color = trace.color || traceColors[tIdx % traceColors.length];
