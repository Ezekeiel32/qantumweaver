...
async def get_parameter_suggestions(previous_job_id: str, current_params: TrainingParameters) -> TrainingParameters:
    """Get parameter suggestions from the HS-QNN advisor service."""
    try:
        response = requests.post(
            'http://your-python-service/predict',
            json={
                'previousJobId': previous_job_id,
                'P_suggested': current_params.to_dict()
            }
        )
        if response.ok:
            data = response.json()
            return TrainingParameters.from_dict(data['suggestedNextTrainingParameters'])
        else:
            print("Warning: Failed to get parameter suggestions, using default parameters")
            return current_params
    except Exception as e:
        print(f"Warning: Error getting parameter suggestions: {e}")
        return current_params

async def train_model(initial_params: Optional[TrainingParameters] = None, previous_job_id: Optional[str] = None):
    ...
    for epoch in range(params.epochs):
        ...
        print(f'Epoch {epoch+1}/{params.epochs}, Validation Accuracy: {val_acc:.2f}%')
    ...
    # Get parameter suggestions for next training run if previous_job_id is provided
    if previous_job_id:
        next_params = await get_parameter_suggestions(previous_job_id, params)
...
if __name__ == "__main__":
    ...
    model, accuracy = asyncio.run(train_model())
    
    # Example usage with custom parameters and previous job ID
    ...
    # )
    # model, accuracy = asyncio.run(train_model(custom_params, previous_job_id="job_123")) 